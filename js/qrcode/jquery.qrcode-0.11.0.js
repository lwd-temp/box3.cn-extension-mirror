...
console.log(settings.width);
    if ($(settings.background).is('img')) {
        ...
        context.fillRect(settings.left, settings.top, settings.width, settings.height);
    }
...
function drawModules(qr, context, settings) {
    ...
    if ($(settings.fill).is('img')) {
        ...
        context.clip();
        context.drawImage(settings.fill, 0, 0, settings.width, settings.height);
        context.restore();
    } else {
...
function createDiv(settings) {
    ...
    var settings_size = settings.width;
